...
const Home = () => {
  ...
  useEffect(() => {
    document.title = authState.isLoggedIn ? `${authState.user.name}'s tasks` : "DevTask";
  }, [authState]);
      ...
      <MainLayout>
        {!isLoggedIn ? (
          <div className='bg-primary text-white h-[40vh] py-8 text-center'>
            <h1 className='text-2xl'> Welcome to Devtask App</h1>
            <Link to="/signup" className='mt-10 text-xl block space-x-2 hover:space-x-4'>
              <span className='transition-[margin]'>Join now to manage your programming tasks easily </span>
